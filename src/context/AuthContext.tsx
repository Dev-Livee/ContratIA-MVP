...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string): Promise<Role> => {
    ...
    try {
      ...
      if (data.refreshToken) localStorage.setItem('contratia_refresh', data.refreshToken);
  ...
  const register = async (payload: RegisterPayload): Promise<Role> => {
    ...
    const body = isEmpresa
      ? { ruc: payload.ruc, razonSocial: payload.razonSocial, email: payload.correo, dniRepresentante: payload.dniRepresentante, representanteLegal: payload.nombreRepresentante, password: payload.password, sector: 'Construccion', telefono: '' }
      : { ruc: payload.ruc, razonSocial: payload.razonSocial, email: payload.correo, dniRepresentante: payload.dniRepresentante, representanteLegal: payload.nombreRepresentante, password: payload.password, tipo: 'MUNICIPALIDAD', distrito: 'Lima', provincia: 'Lima', region: 'Lima', telefono: '', cargo: 'Representante' };
